...
export const EFFECTS: Record<string, EffectMeta> = {
  ...
  triple_points: {
    ...
    apply: async ({ tx, player }) => {
      ...
      return {
        message:
          "Сердце Тьмы бьётся в твоей груди. Следующий «Тяжёлый прогон» (макс. сложность ИЛИ Souls-like) даст ×2 поинтов и поднимет cap до 75.",
      };
    },
  },
  ...
  // ========= BUFF: первая ловушка отскакивает обратно в бросавшего =========
  // Чупа чупс Душика. В отличие от protect_from_trap (просто гасит) —
  // эта реально отражает: эффект применяется к нападавшему.
  trap_reflect: makeMarkerBuff(
    "trap_reflect",
    "Чупа чупс на языке. Следующая брошенная в тебя ловушка отскочит обратно — нападавший сам её и хлебнёт.",
  ),
